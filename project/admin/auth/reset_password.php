...
if (isset($_GET['token'])) {
    ...
    $tokenData = $userModel->verifyResetToken($token);
    ...
    if ($tokenData && strtotime($tokenData['expiry']) > time()) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($password === $confirmPassword && strlen($password) >= 8) {
                ...
                if ($userModel->updatePassword($tokenData['user_id'], $hashedPassword)) {
...
<head>
    ...
    <title>Reset Password - Valera</title>
            ...
            <div class="flex flex-col overflow-y-auto md:flex-row">
                ...
                <div class="flex items-center justify-center p-6 sm:p-12 md:w-1/2">
                    <div class="w-full">
                        ...
                        <?php if ($message): ?>
                            <p class="text-<?php echo strpos($message, 'successfully') !== false ? 'green' : 'red'; ?>-500 text-sm mb-4"><?php echo $message; ?></p>
                        <?php endif; ?>
                        <?php if ($tokenValid): ?>
                            <form method="POST" action="">
                                ...
                                <label class="block mt-4 text-sm">
                                    ...
                                    <input name="confirm_password" type="password" class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" required />
                                </label>
                        ...
                        <?php else: ?>
                            <a href="./login.php" class="text-sm font-medium text-purple-600 dark:text-purple-400 hover:underline">Return to Login</a>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
