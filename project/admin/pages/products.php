...
$products = $productModel->getAllProducts($itemsPerPage, $offset);
?>
    ...
    <div class="w-full overflow-hidden rounded-lg shadow-xs">
                ...
                <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                    <?php if (empty($products)): ?>
                        <tr class="text-gray-700 dark:text-gray-400">
                            <td colspan="7" class="px-4 py-3 text-sm text-center">No products found</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($products as $product): ?>
                            <tr class="text-gray-700 dark:text-gray-400">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    <a href="edit_product.php?id=<?php echo urlencode($product['product_id']); ?>" class="text-blue-600 hover:underline">Edit</a>
                                    <a href="delete_product.php?id=<?php echo urlencode($product['product_id']); ?>" class="text-red-600 hover:underline ml-2">Delete</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
        ...
        <div class="grid px-4 py-3 text-xs font-semibold tracking-wide text-gray-500 uppercase border-t dark:border-gray-700 bg-gray-50 sm:grid-cols-9 dark:text-gray-400 dark:bg-gray-800">
            <span class="flex items-center col-span-3">
                Showing <?php echo min($offset + 1, $totalProducts); ?>-<?php echo min($offset + count($products), $totalProducts); ?> of <?php echo $totalProducts; ?>
            </span>
            ...
            <span class="flex col-span-4 mt-2 sm:mt-auto sm:justify-end">
                <nav aria-label="Table navigation">
                    <ul class="inline-flex items-center">
                        <?php if ($page > 1): ?>
                            <li><a href="?p=products&page=<?php echo $page - 1; ?>" class="px-3 py-1 rounded-md rounded-l-lg focus:outline-none focus:shadow-outline-purple" aria-label="Previous">
                        ...
                        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                            <li><a href="?p=products&page=<?php echo $i; ?>" class="px-3 py-1 rounded-md <?php echo $i === $page ? 'text-white bg-purple-600 border border-r-0 border-purple-600' : ''; ?> focus:outline-none focus:shadow-outline-purple"><?php echo $i; ?></a></li>
                        <?php endfor; ?>
                        <?php if ($page < $totalPages): ?>
                            <li><a href="?p=products&page=<?php echo $page + 1; ?>" class="px-3 py-1 rounded-md rounded-r-lg focus:outline-none focus:shadow-outline-purple" aria-label="Next">
